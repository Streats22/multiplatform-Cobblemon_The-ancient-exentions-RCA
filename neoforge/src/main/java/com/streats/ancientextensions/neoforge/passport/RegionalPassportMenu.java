package com.streats.ancientextensions.neoforge.passport;

import com.streats.ancientextensions.dex.RegionalSurveyData;
import com.streats.ancientextensions.dex.RegionalSurveyService;
import com.streats.ancientextensions.dex.ResearchTier;
import com.streats.ancientextensions.dex.SurveyRegion;
import com.streats.ancientextensions.neoforge.registry.ModMenus;
import net.minecraft.network.RegistryFriendlyByteBuf;
import net.minecraft.server.level.ServerPlayer;
import net.minecraft.world.entity.player.Inventory;
import net.minecraft.world.entity.player.Player;
import net.minecraft.world.inventory.AbstractContainerMenu;
import net.minecraft.world.item.ItemStack;

import java.util.Optional;

public class RegionalPassportMenu extends AbstractContainerMenu {

    public static final int WIDTH = 220;
    public static final int HEIGHT = 200;

    private final boolean stamped;
    private final String regionId;
    private final String holderName;
    private final int caughtSpecies;
    private final int researchPoints;
    private final ResearchTier tier;

    public RegionalPassportMenu(int containerId, Inventory playerInventory, RegistryFriendlyByteBuf extraData) {
        this(
                containerId,
                playerInventory,
                extraData.readBoolean(),
                extraData.readUtf(),
                extraData.readUtf(),
                extraData.readInt(),
                extraData.readInt(),
                ResearchTier.valueOf(extraData.readUtf())
        );
    }

    private RegionalPassportMenu(
            int containerId,
            Inventory playerInventory,
            boolean stamped,
            String regionId,
            String holderName,
            int caughtSpecies,
            int researchPoints,
            ResearchTier tier
    ) {
        super(ModMenus.REGIONAL_PASSPORT.get(), containerId);
        this.stamped = stamped;
        this.regionId = regionId;
        this.holderName = holderName;
        this.caughtSpecies = caughtSpecies;
        this.researchPoints = researchPoints;
        this.tier = tier;
    }

    public static RegionalPassportMenu forPlayer(int containerId, Inventory inventory, ServerPlayer player) {
        RegionalSurveyData data = RegionalSurveyService.get(player);
        Optional<SurveyRegion> origin = data.getSurveyOrigin();
        return new RegionalPassportMenu(
                containerId,
                inventory,
                origin.isPresent(),
                origin.map(SurveyRegion::getId).orElse(""),
                player.getGameProfile().getName(),
                data.getCaughtSpeciesCount(),
                data.getResearchPoints(),
                data.getTier()
        );
    }

    public static void writeExtraData(RegistryFriendlyByteBuf buf, RegionalSurveyData data, ServerPlayer player) {
        Optional<SurveyRegion> origin = data.getSurveyOrigin();
        buf.writeBoolean(origin.isPresent());
        buf.writeUtf(origin.map(SurveyRegion::getId).orElse(""));
        buf.writeUtf(player.getGameProfile().getName());
        buf.writeInt(data.getCaughtSpeciesCount());
        buf.writeInt(data.getResearchPoints());
        buf.writeUtf(data.getTier().name());
    }

    public String getHolderName() {
        return holderName;
    }

    public boolean isStamped() {
        return stamped;
    }

    public Optional<SurveyRegion> getRegion() {
        return SurveyRegion.fromId(regionId);
    }

    public int getCaughtSpecies() {
        return caughtSpecies;
    }

    public int getResearchPoints() {
        return researchPoints;
    }

    public ResearchTier getTier() {
        return tier;
    }

    @Override
    public ItemStack quickMoveStack(Player player, int index) {
        return ItemStack.EMPTY;
    }

    @Override
    public boolean stillValid(Player player) {
        return true;
    }
}
