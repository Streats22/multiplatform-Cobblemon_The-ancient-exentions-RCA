...
- **With Serene Seasons** (`sereneseasons`, optional): migration season = world's current Serene Seasons season (
...
- Routes: `MigrationRoutes.java` — each leg accepts **Regions Unexplored** biomes assigned to that season, plus any
  vanilla `minecraft:*` biome (covers **Regions Unexplored: Expansion** remodels).
- Migratory species (quest + catch credit): `MigrationSpecies.java` — nine species per season; regenerate spawn JSON
  with `scripts/generate_migration_spawns.py`
