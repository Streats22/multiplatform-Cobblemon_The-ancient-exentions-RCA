...
public class FieldSurveyCalendarScreen extends AbstractContainerScreen<FieldSurveyCalendarMenu> {
    ...
    public FieldSurveyCalendarScreen(FieldSurveyCalendarMenu menu, Inventory playerInventory, Component title) {
        super(menu, playerInventory, title);
        this.imageWidth = FieldSurveyCalendarMenu.WIDTH;
        this.imageHeight = FieldSurveyCalendarMenu.HEIGHT;
    ...
    private void buildPages() {
        ...
        for (Component line : menu.getLines()) {
            ...
            for (FormattedCharSequence wrapped : font.split(line, CONTENT_W)) {
                if (current.size() >= maxLines) {
                    pages.add(List.copyOf(current));
                    current = new ArrayList<>();
                }
        ...
        if (pages.isEmpty()) {
            pages.add(List.of(font.split(
                    Component.translatable("ancient_extensions.field_calendar.no_spawns"),
                    CONTENT_W
            ).getFirst()));
        }
    ...
    private void addNavButtons() {
        int footerBtnY = topPos + FOOTER_TOP + (FOOTER_H - JournalNavButton.SPRITE_H) / 2;
        int prevX = leftPos + PAGE_BTN_LEFT_X;
    ...
    protected void renderBg(GuiGraphics graphics, float partialTick, int mouseX, int mouseY) {
        graphics.blit(TEXTURE, leftPos, topPos, 0, 0, imageWidth, imageHeight, TEX_SIZE, TEX_SIZE);
    }
    ...
    protected void renderLabels(GuiGraphics graphics, int mouseX, int mouseY) {
        ...
        Component header = Component.translatable("block.ancient_extensions.field_survey_calendar");
        graphics.drawCenteredString(font, header, cx + 1, 20, GuiTextRender.SLIGHT_BLACK_SHADOW);
        graphics.drawCenteredString(font, header, cx, 19, COLOR_HEADER);

        int y = CONTENT_Y;
        for (FormattedCharSequence line : pages.get(pageIndex)) {
            GuiTextRender.drawStyledSoft(font, graphics, line, CONTENT_X, y);
            y += LINE_HEIGHT;
        }
        ...
        if (pages.size() > 1) {
            ...
            int labelY = FOOTER_TOP + (FOOTER_H - font.lineHeight) / 2;
            graphics.drawString(font, label, labelX + 1, labelY + 1, GuiTextRender.SLIGHT_BLACK_SHADOW, false);
            graphics.drawString(font, label, labelX, labelY, COLOR_PAGE, false);
        }
    }
